...
<div class="secHeaderRow text-center loginHeader">
    <h2 class="secHeader inline">ORDERS</h2>
    <i class="sprite sprite-checkoutIcon3"></i>
</div>
<?php if ( $has_orders ) : ?>

	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table cust_table">
		<thead>
			<tr>
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) {
										echo '<div class="btnHolder"><a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button btn noarrow ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a></div>';
									}
								}
								?>
							<?php endif; ?>
...
<?php else : ?>
    <div class="noMsg inline">
        <?php _e( 'No order has been made yet.', 'woocommerce' ); ?>
    </div>
    <div class="btnHolder pull-right inline">
        <a class="btn" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>"><?php _e( 'Go shop', 'woocommerce' ) ?></a>
    </div>
<?php endif; ?>
